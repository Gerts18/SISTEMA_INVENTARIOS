...
return new class extends Migration
{
    ...
    public function up(): void
    {
        Schema::create('salidas_producto', function (Blueprint $table) {
            $table->increments('salida_producto_id');
            $table->unsignedInteger('salida_inv_id');
            $table->unsignedInteger('producto_id');
            $table->integer('cantidad');
            $table->timestamps();

            $table->foreign('salida_inv_id')->references('salida_inv_id')->on('salidas_inventario')->onDelete('cascade');
